...
void View::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if(m_capture) {
        // Check if dragging
        if (m_drag) {
            float x = 2.f*((float)m_lastX/(float)this->width())-1.f;
            float y = 1.f-2.f*((float)m_lastY/(float)this->height());

            // Drag the shape
            Eigen::Matrix4f screenToWorld = (m_camera.getProjection()*m_camera.getView()).inverse();
            Eigen::Vector4f v = screenToWorld*Eigen::Vector4f(x,y,0.f,1.f);
            v /= v[3];

            Eigen::Vector4f eyeToV = v-Eigen::Vector4f(0,2,-5,1);
            Eigen::Vector4f d_v = m_camera.getView().inverse().transpose()
                    *Eigen::Vector4f((float)deltaX/this->width(),-(float)deltaY/this->height(),0,0.f);

            Eigen::Vector3f d = Eigen::Vector3f(eyeToV[0],eyeToV[1],eyeToV[2]);
            d.normalize();

            m_sim.drag(d,Eigen::Vector3f(v[0],v[1],v[2]),Eigen::Vector3f(d_v[0],d_v[1],d_v[2]));
        }
...
void View::tick()
{
    float seconds = m_time.restart() * TIMESTEP;
